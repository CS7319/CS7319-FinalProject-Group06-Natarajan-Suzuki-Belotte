...
public class JdbcVendorDao implements VendorDao {
    ...
    public List<Vendor> getAllVendors() {
        String sql = "SELECT * FROM vendors ORDER BY is_preferred DESC, name ASC";
        try {
    ...
    public List<Vendor> getPreferredVendors() {
        String sql = "SELECT * FROM vendors WHERE is_preferred = true ORDER BY name ASC";
        try {
            return jdbcTemplate.query(sql, vendorRowMapper);
        } catch (CannotGetJdbcConnectionException e) {
            throw new DaoException("Unable to connect to server or database", e);
    ...
    public List<VendorReview> getReviewsByVendorId(int vendorId) {
        String sql = "SELECT vr.*, u.name AS reviewer_name " +
                ...
                "JOIN users u ON vr.reviewer_email = u.email " +
    ...
    public void addVendorToEvent(int eventId, int vendorId) {
        String sql = "INSERT INTO event_vendors (event_id, vendor_id) VALUES (?, ?) ON CONFLICT DO NOTHING";
        try {
            jdbcTemplate.update(sql, eventId, vendorId);
        } catch (CannotGetJdbcConnectionException e) {
            throw new DaoException("Unable to connect to server or database", e);
        } catch (DataIntegrityViolationException e) {
    ...
    public void removeVendorFromEvent(int eventId, int vendorId) {
        String sql = "DELETE FROM event_vendors WHERE event_id = ? AND vendor_id = ?";
        try {
            jdbcTemplate.update(sql, eventId, vendorId);
        } catch (CannotGetJdbcConnectionException e) {
            throw new DaoException("Unable to connect to server or database", e);
